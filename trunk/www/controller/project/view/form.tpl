                    ...
                    {foreach from=$contentRef key=contentRefId item=contentRefElement}
                        {if $contentRefId == $ElementContentRef}
                            {foreach from=$contentRefElement key=contentRefId2 item=contentRefElement2}
                                <option {if $contentRefId2 == $data->$uid->_id}selected="selected" {/if} value="{$contentRefId2}">{$contentRefElement2.prenom|utf8_decode} {$contentRefElement2.nom|utf8_decode}</option>
                            {/foreach}
                        {/if}
                    {/foreach}
                </select>
