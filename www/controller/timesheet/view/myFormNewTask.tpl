            ...
            {foreach from=$e key=k item=element}
                ...
                <dl class="{if $hidden == true}hidden{else}visible{/if}">
                    ...
                    <dd>
                        ...
                        {if $refT == '13'}
                            {if $uid == 'duree'}
                                <input  class="number {if $element->requis}require{/if}" type="text" name="{$uid}" id="formTask_{$uid}" value="{$data->$uid}{if $valueDefaut != false}{$valueDefaut}{/if}" { if $element->limit != '' }maxlength="{$element->limit}"{/if}/>
                                {$formCustomHour}
                            {else}
